...
async function createItemsFromBoQ(frm) {
    ...
    async function generateComponentHidCode(frm) {
        ...
        try {
            for (let idx = 0; idx < frm.doc.bill_of_quantity_id.length; idx++) {
                let row = frm.doc.bill_of_quantity_id[idx];
    ...
    function generateNewBaseCode(frm) {
        const mainProductCodes = frm.doc.bill_of_quantity_id
    ...
    for (const row of frm.doc.bill_of_quantity_id) {
...
async function updateRowInForm(frm, item_code) {
    ...
    try {
        ...
        for (let idx = 0; idx < frm.doc.bill_of_quantity_id.length; idx++) {
            const row = frm.doc.bill_of_quantity_id[idx];
...
async function createMaterialRequest(frm) {
    ...
    try {
        ...
        frm.doc.bill_of_quantity_id.forEach(row => {
...
// Function to update margins for all rows in bill_of_quantity_id
...
frappe.ui.form.on('ID Project', {
    ...
    bill_of_quantity_id: function(frm, cdt, cdn) {
...
frappe.ui.form.on("Bill of Quantity",{
    before_bill_of_quantity_id_remove(frm,cdt,cdn) {
        console.log("hi")
        let row = locals[cdt][cdn]
        if(row.is_component) {return}
        for (let i of frm.doc.bill_of_quantity_id) {
...
frappe.ui.form.on('ID Project', {
    refresh: function (frm) {
        frm.add_custom_button(__('Auto Fill'), function () {
            ...
            const selected_rows = frm.fields_dict.bill_of_quantity_id.grid.get_selected();
            ...
            const child_fields = Object.keys(frm.fields_dict.bill_of_quantity_id.grid.fields_map);
            ...
            const dialog = new frappe.ui.Dialog({
                ...
                primary_action: (values) => {
                    ...
                    selected_rows.forEach((row) => {
                        ...
                        const field_definition = frm.fields_dict.bill_of_quantity_id.grid.fields_map[values.field_to_update];
                        ...
                        if (field_type === 'Link') {
                            ...
                            if (!frappe.get_doc(field_definition.options, value_to_apply)) {
                                frappe.msgprint(__('Please provide a valid Link value.'));
                                return;
                            }
                        }
                    ...
                    if (changes_applied) {
                        ...
                        frappe.msgprint(__('No rows were updated. Please check field names.'));
                    }
            ...
            dialog.fields_dict.field_to_update.$input.on('change', function () {
                ...
                const field_definition = frm.fields_dict.bill_of_quantity_id.grid.fields_map[selected_field];
                if (field_definition) {
